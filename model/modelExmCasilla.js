...
var Global = require('../server/config.js');
var client = new Client();

// genera la conexión a la base de datos "proyectoSeguridad"
exports.connect = function() {
  client.connect({
    host: Global.mariadb.host,
    user: Global.mariadb.user,
    password: Global.mariadb.password,
    db: Global.mariadb.db
  });
...
exports.getListado = function(cb) {
  var data = []; //
  client.query("SELECT intId, intIdPregunta, strDescripcion, bolCorrecto, fltValor, intTabular FROM tblExmCasilla;;")
    ...
    .on('end', function() {
      cb(data);
      console.log('Obtención de listado datos desde "model_canton" CORRECTA');
    });
};
